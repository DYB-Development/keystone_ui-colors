...
Cutting a release of `keystone_ui-colors` to [RubyGems.org](https://rubygems.org).

## Prerequisites

- A RubyGems.org account with push rights on `keystone_ui-colors`.
...
2. Bump `VERSION` in `lib/keystone_ui/colors/version.rb`.
...
5. Build and inspect the gem before pushing:
   ...
   ```bash
   gem build keystone_ui-colors.gemspec
   tar -xOf keystone_ui-colors-X.Y.Z.gem data.tar.gz | tar -tzf -
   ```

   The listing must include `app/**`, `config/routes.rb`, `lib/generators/**`
   (with the migration templates) and `MIT-LICENSE`.

6. Push, entering the OTP when prompted:

   ```bash
   gem push keystone_ui-colors-X.Y.Z.gem
   ```

7. Tag the release and push the tag:

   ```bash
   git tag -m "keystone_ui-colors X.Y.Z" vX.Y.Z
...
8. Verify from a clean environment:
   ...
   ```bash
   gem install keystone_ui-colors -v X.Y.Z
   ```
